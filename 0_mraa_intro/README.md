...
ロータリーエンコーダーのクリック[^注1]を読みとって割り込みイベントを発生させ、それによってインクリメント・デクリメント(基準の数に+1したり-1したりする)された数値をコンソールに表示します。
先ほどまでの例と異なり、エンコーダーがトリガを発生させるので、エンコーダーを回している間はコンソールに値が出力されるようになります。

[^注1]: ロータリーエンコーダーにはプッシュスイッチが一体化しているものもあるので紛らわしい言い回しですが、ここではプッシュスイッチを押す動作の意味ではなく、ロータリーエンコーダーの回転機構上の最小動作単位の意味として使っています。
